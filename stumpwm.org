...
* Set up
Set the module directory. Using a cloned folder from github.
...
Set up =slynk= to connect to =sly= in emacs
From http://www.kaashif.co.uk/2015/06/28/hacking-stumpwm-with-common-lisp/index.html

In order for this to work, slynk needs to be installed
#+begin_src lisp
  (ql:quickload "slynk")
#+end_src
...
In emacs, use =M-x my/stumpwm-connect= to launch the repl. Set up stumpwm with =(in-package :stumpwm)= and eval away!
