...
Using this so I can =M-x package-list-packages= to list browse packages on MELPA, otherwise =straight= doesnt need it.
#+BEGIN_SRC emacs-lisp
  ;; (require 'package)
...
#+END_QUOTE
#+BEGIN_SRC emacs-lisp
  (setq straight-use-package-prepare-functions t)
#+END_SRC
