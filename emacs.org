...
#+END_SRC
** Org Mode
Each of these sections contains functions that are called within the =:hook= or =:config= part of the =(use-package org)= block
*** Prettify
...
Set up fonts and faces. Most of these are for for =variable-pitch-mode= to make things =monospaced=. The =org-level-*= stuff makes headings bigger.
#+BEGIN_SRC emacs-lisp
  (defun my-org-faces ()
...
The todo states are largely taken from from http://doc.norang.ca/org-mode.html
#+BEGIN_SRC emacs-lisp
...
- [ ] new org fixes [0/2]
  - [ ] custom structure templates arent loading
