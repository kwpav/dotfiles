...
- it's intregated well with =use-package=
...
- =org-bullets= provides good looking bullets for the header
...
From http://blog.lujun9972.win/emacs-document/blog/2018/10/22/ricing-up-org-mode/index.html and https://zzamboni.org/post/beautifying-org-mode-in-emacs/
...
Customize some faces,  mainly for =variable-pitch-mode= to make things =monospaced=. The =org-level-*= stuff makes headings bigger.
#+BEGIN_SRC emacs-lisp
  (defun my-org-faces ()
...
*NOTE* Using the built in org version because =org-indent-mode= and my agenda are not working with the latest version
#+BEGIN_SRC emacs-lisp
  (use-package org
    :straight nil
    :general
...
- [-] new org fixes [2/3]
  - [X] =org-indent-mode= doesnt work in 9.2.6, it works in 9.2.5 and the built in version
    - used the commit hash for 9.2.5 in =straight/version/default.el= =
  - [X] custom structure templates arent loading
    - this is working in version 9.2.5
      - will probably need to use =org-tempo= and fix this again whenever i update org
    - use yasnippet?
  - [ ] figure out why org agenda isnt working in 9.2.5 or 9.2.6
    - it works with the built-in version of org, using that for now
