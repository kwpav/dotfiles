...
Set up =straight-x.el= - this allows provides functionality to pin package versions. This needs to be done before straight is loaded
#+BEGIN_SRC emacs-lisp
...
Now that =straight= has been loaded, we can load =straight-x=
#+BEGIN_SRC emacs-lisp
...
Using the built in org version because =org-indent-mode= and my agenda are not working with the latest version
#+BEGIN_SRC emacs-lisp
