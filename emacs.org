...
Use UTF-8 for (almost) everything. This should also fix flycheck errors with python-flake8.
#+BEGIN_SRC emacs-lisp
  (use-package mule
    :straight nil
    :config
...
Set up so-long mode to handle files with large lines. If a line in a file is too long, this will turn off syntax highlighting, which causes slowness when using files with long lines.
#+begin_src emacs-lisp
  (use-package so-long
    :straight nil
    :config
